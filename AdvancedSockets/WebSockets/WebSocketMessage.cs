...
namespace AdvancedSockets.WebSockets
{
    public class WebSocketMessage
    {
        ...
        public WebSocketMessage(byte[] buffer, WebSocketOpcode opcode, bool masked)
        {
            this.buffer = buffer;
            this.Opcode = opcode;
            this.Masked = masked;
        ...
        public byte[] Encode()
        {
            ...
            var bufferLengthBinary = (buffer == null ? 0 : buffer.Length).IntToBinaryString();
            ...
            if (buffer == null || buffer.Length < 126)
            {
                result.Add((maskedByte + bufferLengthBinary.Substring(1)).BinaryStringToByte());
            }
            ...
            if (buffer != null && buffer.Length >= 126 && buffer.Length < 65535)
            {
                result.Add((maskedByte + "1111110").BinaryStringToByte());
                result.Add(bufferLengthBinary.PadLeft(16, '0').Substring(0, 8).BinaryStringToByte());
                result.Add(bufferLengthBinary.PadLeft(16, '0').Substring(8, 8).BinaryStringToByte());
            }
            ...
            if (buffer != null && buffer.Length >= 65535)
            {
                result.Add((maskedByte + "1111111").BinaryStringToByte());
                result.Add(bufferLengthBinary.PadLeft(32, '0').Substring(0, 8).BinaryStringToByte());
                result.Add(bufferLengthBinary.PadLeft(32, '0').Substring(8, 8).BinaryStringToByte());
                result.Add(bufferLengthBinary.PadLeft(32, '0').Substring(16, 8).BinaryStringToByte());
                result.Add(bufferLengthBinary.PadLeft(32, '0').Substring(24, 8).BinaryStringToByte());
            }
            ...
            if (buffer != null)
            {
                if (Masked)
                {
                    ...
                    {
                        result.Add((byte)(buffer[i] ^ maskBytes[i % 4]));
                    }
                }
                else
                {
