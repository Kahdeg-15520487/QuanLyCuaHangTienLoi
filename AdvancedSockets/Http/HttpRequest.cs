...
namespace AdvancedSockets.Http
{
    public class HttpRequest
    {
        ...
        internal HttpRequest(Socket socket) : this()
        {
            Socket = socket;
        }
        ...
        private void ParseBody(List<byte> data)
        {
            ...
            // Both the GET and OPTIONS method never have a body
            ...
            {
                contentType = Headers["content-type"].Replace("\r", "").Replace("\n", "").Replace(" ", "").Split(';');
            }
        ...
        {
            var dataString = Encoding.ASCII.GetString(data.ToArray());
            ...
            foreach (var b in data)
            {
                ...
                if (lineString.StartsWith("--" + boundary) && bodyBytes.Count > 0)
                {
                    ...
                    else
                    {
                        ...
                        {
                            var contentString = Encoding.ASCII.GetString(bodyBytes.ToArray());
                        ...
                        {
                            var value = Encoding.ASCII.GetString(bodyBytes.ToArray());
                ...
                {
                    contentType = lineString.Split(':')[1].Trim();
                }
                else if (lineString == "\r\n")
                {
        ...
        public byte[] GenerateRaw()
        {
            ...
            if (Body != null)
            {
                if (Body.Files != null)
                {
                    ...
                    if (Body.KeyValues != null)
                    {
                        foreach (var entry in Body.KeyValues)
                        {
                            ...
                            body.AddRange(crlf);
                            body.AddRange(Encoding.ASCII.GetBytes(contentDisposition));
                            body.AddRange(Encoding.ASCII.GetBytes(contentType));
                            body.AddRange(crlf);
                            body.AddRange(Encoding.ASCII.GetBytes(entry.Value));
                        }
                    }
                ...
                else if (Body.KeyValues != null)
                {
                    ...
                    foreach (var entry in Body.KeyValues)
                    {
                        ...
                        var bytes = Encoding.ASCII.GetBytes(keyValue);
            ...
            result.AddRange(Encoding.ASCII.GetBytes($"{Method.ToString().ToUpper()} {Path} HTTP/1.1\r\n"));

            // Add all headers
            result.AddRange(Encoding.ASCII.GetBytes(Headers.ToString()));

            // Add a newline to mark the body
            result.AddRange(crlf);

            // Now write down the body
